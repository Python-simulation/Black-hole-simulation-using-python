...
nbr_offset=10    #number of image needed with a constant offset between (must be changed to specify a precise angle)
...
def splin(x_pivot,f_pivot):
    """From : source unknown 
    Display the interpolation (optional)"""
...
def matrices_creation(interpolation):
    ...
    debut=time.process_time()
    if FOV<FOV_img:
        print("\nmatrix estimation time:",round(1.36*5.5*10**(-5)*(FOV/FOV_img)*(axe_X*axe_Y-(2*(180-last_angle)/360*axe_X)**2),1)) #modif les FOV en x et y car diff si on a fov_x=360 fov_y=180
    else:
        print("\nmatrix estimation time:",round(1.36*5.5*10**(-5)*(axe_X*axe_Y-(2*(180-last_angle)/360*axe_X)**2),1)) #modif les FOV en x et y car diff si on a fov_x=360 fov_y=180

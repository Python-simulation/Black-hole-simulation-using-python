...
def solver(D, alpha):
    ...
    phi = sol.t
    r = abs(1/sol.y[0,:]) #must use this because solver can't be stop before infinity because negative
...
#    print(phi, theta)
    ...
    if seen_angle > 180: #only right because spherical problem (not right with Kerr)
        ...
        try:
#            deviated_angle=360-interpolation(seen_angle) #old version 2x slower
        ...
        try:
#            deviated_angle = interpolation(seen_angle) #old version 2x slower
...
def check_matrices():
    ...
    if use_matrix is True: # Check if matrix exist
    ...
    if x_file is True and y_file is True:
...
#        print("graphique fermé")
    try:
        if float(size.get()) <= 0:
            ...
            img2 = Image.new( 'RGBA', (axe_X, axe_Y)) #creat a transparent image (outside of the function to recover info if must stop loop)
            img2 = img_pixels(img_debut, img2)
#            if fixed_background is True:  #must be changed if want to work here
#                img2 = img_offset_X(img2,-offset_X_tot)  # if want a fixed background and moving black hole
            img = ax.imshow(img2)
            fig.canvas.draw()
            print("Done !")
...
# =============================================================================
image_name = 'images\\milkyway.jpg'
[img_original, img_debut] = open_image(image_name)
